...
export default function HeroSection() {
  ...
  const triggerGate = useCallback(() => {
    ...
    // Step 3: "LUCA FILMS" fades out after 3s
    setTimeout(() => setLucaFilmsVisible(false), 4200);

    // Step 4: unlock scroll after video has played a moment
    setTimeout(() => {
      setPhase("unlocked");
      document.body.style.overflow = "";
      document.body.style.touchAction = "";
  ...
  return (
    <>
      ...
      {/* ── GATE: fixed full-screen, sits above everything ── */}
      <AnimatePresence>
        {showGate && (
          <motion.div
            key="gate"
      ...
      {/* ── VIDEO: fades in behind the gate ── */}
      <AnimatePresence>
        {showVideo && (
          <motion.div
            key="video"
          ...
          >
            ...
            {/* LUCA FILMS overlay — appears then fades */}
            <AnimatePresence>
              {lucaFilmsVisible && (
                <motion.div
                  key="lucafilms"
      ...
      {/* ── ACTUAL HERO SECTION (scrollable content, below the gate) ── */}
      <section
        className="relative min-h-[100dvh] w-full flex flex-col items-center justify-center bg-black overflow-hidden"
        id="hero"
      >
        {/* static grain for this section */}
        <div
          className="absolute inset-0 pointer-events-none opacity-20 mix-blend-screen"
          style={{ backgroundImage: NOISE_SVG, backgroundSize: "200px 200px" }}
        />

        <div className="relative z-10 w-full pt-28 pb-24 px-4 flex flex-col items-center border-t border-white/10">
